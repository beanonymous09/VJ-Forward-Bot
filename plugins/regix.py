...
async def pub_(bot, message):
    ...
    try:
       forwarded = sts.get('total_files') + 1  # Increase forwarding count
       sts.add('total_files', 1)
       msg = await client.send_message(i.TO, f"✅ sᴜᴄᴄᴇғᴜʟʟʏ Fᴡᴅ : <code>{forwarded}</code>")
...
async def copy(user, bot, msg, m, sts):
    try:
        if msg.get("media"):
            ...
            if msg.get("media") == "photo":
                new_caption = '<blockquote><b><a href="https://t.me/II_Way_to_Success_II">📷 Default Image Caption 📷</a></b></blockquote>'
            else:
                new_caption = msg.get("caption")  # Keep original caption for other media types
